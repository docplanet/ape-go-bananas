// The chat pane, over the bridge: one session, streamed updates rendered as
// they arrive, the agent's own config options and modes as selectors, and
// the permission prompt answered over the reverse channel. Same code for an
// ACP agent and for the embedded OpenRouter loop -- the sidecar hides which
// is which. app/src/chat.ts, with the Tauri listeners swapped for the
// bridge's.
//
// One bridge carries one events stream, so the notification and request
...
export function mountChat(host: HTMLElement, sidecar: SidecarClient, bus: Bus, conn: ConnectResult, say: (t: string, e?: boolean) => void): Chat {
  ...
  selectors.addEventListener('change', async (e) => {
    ...
    try {
      ...
      say(err instanceof BridgeError ? err.message : String(err), true);
    }
  });
  ...
  host.querySelector<HTMLFormElement>('#composer')!.onsubmit = async (e) => {
    ...
    try {
      ...
      append('tool', `error: ${err instanceof BridgeError ? err.message : String(err)}`, true);
    } finally {
      busy = false;
      current = null;
